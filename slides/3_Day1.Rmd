...
- [`{knitr}`](https://yihui.org/knitr/) (an R package) runs the code and adds the output to the text

- `knitr` creates a `.md` document

- [Pandoc](https://pandoc.org/) (+ LaTeX) converts the markdown document to the desired output format
...
**For code chunks that have figures as output**
...
- Is used by Pandoc, rmarkdown and knitr at different stages
...
- Inline R code can print the current date at time of knitting the document
...
- Output formats have options than can be specified in the YAML header
...
- Use the [rmarkdown cheat sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/master/rmarkdown-2.0.pdf) as reference for the most important options
...
Document parameter values can be use in R code (inline or code chunks) with `params$param_name`


````md
`r ''````{r iris-figure, fig.width=4, fig.height=4}
