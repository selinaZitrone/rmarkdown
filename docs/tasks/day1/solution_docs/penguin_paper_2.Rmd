...
knitr::opts_chunk$set(
  echo = FALSE, # code is not included in output
...
[![The image](https://raw.githubusercontent.com/allisonhorst/palmerpenguins/main/man/figures/lter_penguins.png)](https://raw.githubusercontent.com/allisonhorst/palmerpenguins/main/man/figures/lter_penguins.png)

![Artwork by [Allison Horst](https://twitter.com/allison_horst)](img/lter_penguins.png)


```{r include-penguins, out.width="60%", fig.align = "center", fig.cap="Illustration of the three penguin species by [Allison Horst](https://twitter.com/allison_horst)"}
...
The mean weight of all penguin species is `r round(mean(penguins$body_mass_g, na.rm = TRUE), 0)`. *Gentoo* penguins have an average weight of `r round(gentoo_mean,0)` g, *Adelie* penguins of `r round(adelie_mean, 0)` g and *Chinstrap* penguins of `r round(chinstrap_mean, 0)` g.

The figure below shows that *Gentoo* penguins have the highest body mass.

```{r histogram-body, fig.cap="Histogram of weight of the three penguin species.", fig.width=4, fig.height=3, fig.align='center', echo = TRUE}
...
```{r scatterplot-bill, fig.cap="Scatter plot with regression lines showing the relationship between bill length and bill depth for the 3 penguin species", fig.align="center", echo = TRUE}
scatter <- ggplot(
  data = penguins,
  aes(
    x = bill_length_mm,
...
# This works for all outputs
# scatter

# This only works for HTLM output and makes an interactive plot
