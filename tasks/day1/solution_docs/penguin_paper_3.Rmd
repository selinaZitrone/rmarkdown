...
The mean flipper length of all penguin species is `r round(mean(penguins$flipper_length_mm, na.rm = TRUE),1)` ($\pm$ `r round(mean(penguins$flipper_length_mm, na.rm = TRUE),1)`). 
...
The Gentoo species has the highest flipper length (mean `r round(gentoo_mean,1)`), followed by 
the Chinstrap species (mean `r round(chinstrap_mean,1)`). The Adelie species as the lowest
average flipper length (mean `r round(adelie_mean,1)`).
